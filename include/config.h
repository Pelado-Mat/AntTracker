...
//#define Compass_Declination -0.123
...
// from binding_phrase) Insert the six numbers between the curly brackets below
  ...
  const uint8_t myUID[6] = {57,182,156,147,161,105}; // this is my test UID. It will not work for you
#endif
//=============================================================================================
...
//=========================================================================   
  ...
  // For info: Avoid SPI pins - generally   CS=5    MOSI=23   MISO=19   SCK=18  
    ...
    int8_t in_rxPin =         0;  // uart1
    #define in_txPin          10 
    #if (HEADINGSOURCE == 4)  // Tracker box GPS 
      int8_t boxgps_rxPin =     16;  // uart2 for tracker box GPS if applicable
      #define boxgps_txPin       17  
    #endif  
    bool in_invert = false;         // ONLY FOR FrSky S.Port, NOT F.Port, NOT MAVLINK
    #define set_Pin           25   // LOW == pushed    
    #define StatusLed         26   // Off=No good GPS yet, flashing=good GPS but home not set yet, solid = ready to track
    #if defined SERVOS
    ...
    /* Below please choose either Touch pin-pair or Digital pin-pair for display scrolling
      *  Pin == -1 means the pin-pair is not used
      */ 
    #define Pup           34       // 34 Board Button 1 to scroll the display up
